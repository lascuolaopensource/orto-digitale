...
async function getRecipeAuthor(recipe: any): Promise<string> {
  const t = await getTranslations()

  // Extract author from rich text field if it exists
  if (recipe && recipe.autore) {
    if (typeof recipe.autore === 'object' && recipe.autore?.root?.children?.[0]?.text) {
      const text = recipe.autore.root.children
